...
public class AccountTest extends TestCase {
  ...
  public void testSendMessage() {
    ...
    try {
      noAcc.sendMessage(new Message("123",
          "This should not work because the accounts doesn't exist", noAcc2, noAcc), noAcc2);
      fail();
    } catch (IllegalStateException e) {
    ...
    try {
      testAccFrom.sendMessage(new Message("testMessage", "Message", testAccTo, testAccFrom),
          testAccTo);
    } catch (IllegalStateException e) {
      fail();
    } catch (IOException e) {
  ...
  private void deleteLastLineOfFile() {
    try {
      ...
      RandomAccessFile f = new RandomAccessFile(
          new File(AccountIO.resourceFilepath + AccountIO.usersFilename), "rw");
